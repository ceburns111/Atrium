...
- **Services** use feature folders, `Map*Endpoints` on route groups with `.WithTags` and `RequireAuthorization`; hosts wire the shared `Atrium.ServiceDefaults` extensions (`AddAtriumTelemetry`, `AddAtriumJwtAuth()` — which also registers the `admin` policy — and `MapAtriumApiDocs(title)`). Auth matrix: Catalog reads anonymous; orders authenticated; Catalog writes and Reports reads `admin`.
