...
/// runs a turn over a swappable <c>IChatClient</c> (the service's own <see cref="CannedChatClient"/>),
/// so the agent construction — including tool registration — is exercised without a real model. The
/// agent is built with only an <see cref="IHttpContextAccessor"/> (it resolves its scoped tools per
/// call), so no request scope is needed for this turn: the fake client replies without invoking any tool.
/// </summary>
public class SupportAgentTests
{
    ...
    {
        const string cannedReply = "PONG from the fake model.";
